...
assert viz.check_connection(), f"Could not connect to visdom. Run $visdom -port {port} first"
...
train_dataset = TileDataset(os.path.join(root_dir, "label/"), normalizer, train_idx) # change to labelled dir
val_dataset = TileDataset(os.path.join(root_dir, "label/"), normalizer, val_idx) # change to labelled dir
test_dataset = TileDataset(os.path.join(root_dir, "label/"), normalizer, test_idx) # change to labelled dir
...
init_batch = get_init_batch(lab_trainloader, 1)
...
try:
    ...
    for epoch in range(1, 1 + nb_epochs):
        ...
        train_cm, train_acc = train(model, optimizer, epoch, lr, trainloader, unlab_trainloader, viz, train_log, class_weights, use_cuda, classification_weight)
